...
interface CounterListItemProps {
    ...
    onEdit?: () => void;
    onDelete?: () => void;
    onRequestToggleArchive?: () => void; // Renamed
    onShare?: () => void;
}
...
export function CounterListItem({
    ...
    useEffect(() => {
        ...
        if (!isArchived && startDate instanceof Date && !isNaN(startDate.getTime())) {
            ...
            setCurrentTimeDiff(calculateTimeDifference(startDate, archivedDate));
        }
        return () => { if (intervalId) clearInterval(intervalId); };
    }, [startDate, isArchived, archivedDate]);

    const finalTimeDiff = useMemo<TimeDifference>(() => {
    ...
    const handleToggleArchive = () => onRequestToggleArchive?.(); // Use prop

    // Action Buttons
    const sharedIconSize = "1rem";
    const sharedActionIconProps = { variant: "subtle", size: "sm", radius: "xl" };
    const getActionIconProps = (color?: string) => ({ ...sharedActionIconProps, color: color || (colorScheme === 'dark' ? theme.colors.dark[1] : theme.colors.gray[6]), style: { transition: 'color 0.2s ease' } });

    const shareButton = onShare && !isArchived && !counter.isPrivate && ( <Tooltip withinPortal label="Copy Share Link" withArrow position="top" openDelay={300}><ActionIcon {...getActionIconProps('teal')} onClick={handleShare}><IconShare3 size={sharedIconSize} stroke={1.5}/></ActionIcon></Tooltip> );
    const editButton = isOwnerView && onEdit && ( <Tooltip withinPortal label="Edit" withArrow position="top" openDelay={300}><ActionIcon {...getActionIconProps(theme.primaryColor)} onClick={handleEdit}><IconPencil size={sharedIconSize} stroke={1.5}/></ActionIcon></Tooltip> );
    const deleteButton = isOwnerView && onDelete && ( <Tooltip withinPortal label="Delete" withArrow position="top" openDelay={300}><ActionIcon {...getActionIconProps("red")} onClick={handleDelete}><IconTrash size={sharedIconSize} stroke={1.5}/></ActionIcon></Tooltip> );
    const archiveButton = isOwnerView && onRequestToggleArchive && ( <Tooltip withinPortal label={isArchived ? "Unarchive" : "Archive"} withArrow position="top" openDelay={300}><ActionIcon {...getActionIconProps("blue")} onClick={handleToggleArchive}>{isArchived ? <IconArchiveOff size={sharedIconSize} stroke={1.5}/> : <IconArchive size={sharedIconSize} stroke={1.5}/>}</ActionIcon></Tooltip> );

    return (
        <Box p="xs" mb={4} style={(theme) => ({ border: `1px solid ${theme.colors[colorScheme === 'dark' ? 'dark' : 'gray'][colorScheme === 'dark' ? 5 : 2]}`, borderRadius: theme.radius.sm, backgroundColor: isArchived && isOwnerView ? (colorScheme === 'dark' ? theme.colors.dark[8] : theme.colors.gray[1]) : (colorScheme === 'dark' ? theme.colors.dark[6] : theme.white), opacity: isArchived && isOwnerView ? 0.7 : 1, transition: 'background-color 0.2s ease', '&:hover': { backgroundColor: colorScheme === 'dark' ? theme.colors.dark[5] : theme.colors.gray[0] } })}>
                             ...
                             <Menu shadow="sm" width={160} position="bottom-end" withArrow withinPortal>
                                <Menu.Target><ActionIcon {...getActionIconProps()} aria-label="More actions"><IconDotsVertical size="1rem" stroke={1.5} /></ActionIcon></Menu.Target>
                                 <Menu.Dropdown miw={150}>
                                    {editButton && <Menu.Item leftSection={<IconPencil size={14} stroke={1.5}/>} onClick={handleEdit}>Edit</Menu.Item>}
                                    {archiveButton && <Menu.Item leftSection={isArchived ? <IconArchiveOff size={14}/> : <IconArchive size={14} />} onClick={handleToggleArchive}>{isArchived ? "Unarchive" : "Archive"}</Menu.Item>}
                                    {shareButton && !counter.isPrivate && <Menu.Item leftSection={<IconShare3 size={14} color={theme.colors.teal[6]}/>} onClick={handleShare}>Copy Share Link</Menu.Item>}
                                    {(editButton || archiveButton || shareButton) && deleteButton && <Menu.Divider />}
                                    {deleteButton && <Menu.Item color="red" leftSection={<IconTrash size={14}/>} onClick={handleDelete}>Delete</Menu.Item>}
                                </Menu.Dropdown>
                            </Menu>
