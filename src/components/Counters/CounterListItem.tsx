           ...
           <Group justify="space-between" wrap="nowrap" gap="sm">
                ...
                {isLinkable ? (
                 ...
                 <Group gap={isMobile ? 2 : 4} wrap="nowrap" justify="flex-end" style={{ flexShrink: 0 }} onClick={(e) => e.stopPropagation()}>
                     {isOwnerView ? ( isMobile ? ( <Menu shadow="sm" width={160} position="bottom-end" withArrow withinPortal> <Menu.Target> <ActionIcon {...getActionIconProps()} aria-label="More actions"> <IconDotsVertical size="1rem" stroke={1.5} /> </ActionIcon> </Menu.Target> <Menu.Dropdown miw={150}> {editButton && <Menu.Item leftSection={<IconPencil size={14}/>} onClick={handleEdit}>Edit</Menu.Item>} {archiveButton && <Menu.Item leftSection={isArchived ? <IconArchiveOff size={14}/> : <IconArchive size={14} />} onClick={handleToggleArchive}>{isArchived ? "Unarchive" : "Archive"}</Menu.Item>} {shareButton && <Menu.Item leftSection={<IconShare3 size={14} color={theme.colors.teal[6]}/>} onClick={handleShare}>Copy Public Link</Menu.Item>} {(editButton || archiveButton || shareButton) && deleteButton && <Menu.Divider />} {deleteButton && <Menu.Item color="red" leftSection={<IconTrash size={14}/>} onClick={handleDelete}>Delete</Menu.Item>} </Menu.Dropdown> </Menu> )
