             ...
             <Box mt="auto">
                 ...
                 <Group justify="space-between" align="center">
                     ...
                     <Group gap={isMobile ? 4 : 8} wrap="nowrap" style={{ flexShrink: 0 }} onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
                         {isOwnerView ? ( <> {archiveButton} {isMobile ? ( <Menu shadow="md" width={180} position="bottom-end" withArrow> <Menu.Target> <ActionIcon {...getActionIconProps()} aria-label="More actions"> <IconDotsVertical size="1.1rem" stroke={1.5} /> </ActionIcon> </Menu.Target> <Menu.Dropdown> {shareButton && <Menu.Item leftSection={<IconShare3 size={14} color={theme.colors.teal[6]} />} onClick={handleShare}>Copy Public Link</Menu.Item>} {editButton && <Menu.Item leftSection={<IconPencil size={14} color={theme.colors[theme.primaryColor][6]} />} onClick={handleEdit}>Edit</Menu.Item>} {(editButton || shareButton) && deleteButtonJsx && <Menu.Divider />} {deleteButtonJsx && <Menu.Item color="red" leftSection={<IconTrash size={14} />} onClick={handleDelete}>Delete</Menu.Item>} </Menu.Dropdown> </Menu> ) : ( <> {editButton} {shareButton} {deleteButtonJsx} </> )} </> )
                          : ( shareButton || null )}
                     </Group>
                 </Group>
             </Box>
